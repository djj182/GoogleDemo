...
find $CLIENT_ROOT_PATH/IF/DayZClasses -name "*.cpp" ! -name "AllSource**.cpp"|awk '{print $1,"\\"}' >> Sources.mk
...
find $CLIENT_ROOT_PATH/IF/DayZClasses -type d -not -wholename '*.svn*' -not -wholename '*ChartIOS*' -not -wholename '*WrapperChatIOS*'|awk '{printf "$(LOCAL_PATH)/%s \\\n",$1}' >> Includes.mk
