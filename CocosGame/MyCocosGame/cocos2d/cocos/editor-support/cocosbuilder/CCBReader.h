...
#define CCBI_PREFIX "ccbi/"  //modify by owner - caibird
#define HDCCBI_PREFIX "hdccbi/" //modify by owner - caibird
...
//    cocos2d::Node* readNodeGraphFromData(std::shared_ptr<cocos2d::Data> data, cocos2d::Ref *pOwner, const cocos2d::Size &parentSize);
    cocos2d::Node* readNodeGraphFromData(unsigned char* udata, cocos2d::Ref *pOwner, const cocos2d::Size &parentSize);
    /**
     @lua NA
     */
...
//    std::string readCachedString();
    std::string const& readCachedString();
...
private:
    ...
    bool readKeyframe2(CCBSequenceProperty* pro);
...
class CC_DLL CacheDataMgr
{
public:
    static CacheDataMgr* getInstance();
    bool addCacheData(std::string const& fullpath, int expire_seconds = -1);
    bool addCacheData(std::string const& fullpath, unsigned char* d, int s, int expire_seconds);
    bool getCacheData(std::string const& fullpath, CacheDataCCB& pData);
    //是否能回收
    void setCanRelease(std::string const& fullpath, bool canRelease);
    void clearExpireCacheData();
    void clearCacheData();
    
    typedef std::recursive_mutex LockType;
    LockType m_cacheLock;
    std::map<std::string, CacheDataCCB> m_mapCacheData;
};
